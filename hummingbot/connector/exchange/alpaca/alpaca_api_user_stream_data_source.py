...
class AlpacaAPIUserStreamDataSource(UserStreamTrackerDataSource):
    MAX_RETRIES = 20
    ...
    def __init__(
        ...
        super().__init__()
        self._api_factory = api_factory or alpaca_utils.build_api_factory()
        self._rest_assistant = None
        self._ws_assistant = None
        self._alpaca_auth: AlpacaAuth = alpaca_auth
    ...
    async def _authenticate(self, ws: WSAssistant):
        ...
        """
        try:
            auth_payload: Dict[str, Any] = self._alpaca_auth.get_ws_auth_payload(alpaca_utils.get_ms_timestamp())
    ...
    async def _subscribe_to_channels(self, ws: WSAssistant):
        ...
        try:
            ...
            self.logger().error("Error occured during subscribing to Alpaca private channels.", exc_info=True)
            raise
        ...
        while True:
            try:
                ...
                try:
                    ...
                    self.logger().info("Alpaca WebSocket already connected.")
                ...
                self.logger().error(
                    "Unexpected error with Alpaca WebSocket connection. Retrying after 30 seconds...",
                    exc_info=True
                )
                await ws.disconnect()
                await asyncio.sleep(30.0)
