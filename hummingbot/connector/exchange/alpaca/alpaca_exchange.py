...
from hummingbot.connector.exchange.alpaca import alpaca_constants as CONSTANTS
from hummingbot.connector.exchange.alpaca import alpaca_utils
from hummingbot.connector.exchange.alpaca.alpaca_auth import AlpacaAuth
from hummingbot.connector.exchange.alpaca.alpaca_in_flight_order import AlpacaInFlightOrder
from hummingbot.connector.exchange.alpaca.alpaca_order_book_tracker import AlpacaOrderBookTracker
from hummingbot.connector.exchange.alpaca.alpaca_user_stream_tracker import AlpacaUserStreamTracker
...
class AlpacaExchange(ExchangeBase):
    """
    AlpacaExchange connects with AlpacaExchange exchange and provides order book pricing, user account tracking and
    ...
    def __init__(self,
        ...
        """
        :param alpaca_api_key: The API key to connect to private Alpaca APIs.
        :param alpaca_secret_key: The API secret.
        :param trading_pairs: The market trading pairs which to track order book data.
        :param trading_required: Whether actual trading is needed.
        """
        super().__init__()
        self._api_factory = alpaca_utils.build_api_factory()
        self._rest_assistant = None
        self._trading_required = trading_required
        self._trading_pairs = trading_pairs
        self._alpaca_auth = AlpacaAuth(api_key=alpaca_api_key,
                                         secret_key=alpaca_secret_key)
        self._throttler = AsyncThrottler(CONSTANTS.RATE_LIMITS)
        self._order_book_tracker = AlpacaOrderBookTracker(
            throttler=self._throttler, trading_pairs=trading_pairs
        )
        self._user_stream_tracker = AlpacaUserStreamTracker(
            throttler=self._throttler, alpaca_auth=self._alpaca_auth, trading_pairs=trading_pairs
        )
        self._ev_loop = asyncio.get_event_loop()
        self._shared_client = None
        self._poll_notifier = asyncio.Event()
        self._last_timestamp = 0
        self._in_flight_orders = {}  # Dict[client_order_id:str, AlpacaInFlightOrder]
    ...
    @property
    def in_flight_orders(self) -> Dict[str, AlpacaInFlightOrder]:
    ...
    def restore_tracking_states(self, saved_states: Dict[str, any]):
        ...
        self._in_flight_orders.update({
            key: AlpacaInFlightOrder.from_json(value)
        ...
        while True:
            try:
                ...
                self.logger().network(f"Unexpected error while fetching trading rules. Error: {str(e)}",
                                      exc_info=True,
                                      app_warning_msg="Could not fetch new trading rules from Alpaca. "
    ...
    def _format_trading_rules(self, symbols_details: Dict[str, Any]) -> Dict[str, TradingRule]:
        ...
        for rule in symbols_details["data"]["symbols"]:
            try:
                trading_pair = alpaca_utils.convert_from_exchange_trading_pair(rule["symbol"])
    ...
    async def _api_request(self,
        ...
        async with self._throttler.execute_task(path_url):
            ...
            headers = self._alpaca_auth.get_headers(alpaca_utils.get_ms_timestamp(), params, auth_type)
    ...
    def buy(self, trading_pair: str, amount: Decimal, order_type=OrderType.MARKET,
        ...
        """
        order_id: str = alpaca_utils.get_new_client_order_id(True, trading_pair)
    ...
    def sell(self, trading_pair: str, amount: Decimal, order_type=OrderType.MARKET,
        ...
        :returns A new internal order id
        """
        order_id: str = alpaca_utils.get_new_client_order_id(False, trading_pair)
    ...
    async def _create_order(self,
        ...
        try:
            ...
            api_params = {"symbol": alpaca_utils.convert_to_exchange_trading_pair(trading_pair),
            ...
            self.logger().network(
                f"Error submitting {trade_type.name} {order_type.name} order to Alpaca for "
                f"{amount} {trading_pair} "
                f"{price}.",
                exc_info=True,
    ...
    def start_tracking_order(self,
        ...
        """
        self._in_flight_orders[order_id] = AlpacaInFlightOrder(
    ...
    async def _execute_cancel(self, trading_pair: str, order_id: str) -> str:
        ...
        try:
            ...
            response = await self._api_request(
                ...
                {"symbol": alpaca_utils.convert_to_exchange_trading_pair(trading_pair),
            ...
            self.logger().network(
                ...
                exc_info=True,
                app_warning_msg=f"Failed to cancel the order {order_id} on Alpaca. "
        ...
        while True:
            try:
                ...
                self.logger().network("Unexpected error while fetching account updates.",
                                      exc_info=True,
                                      app_warning_msg="Could not fetch account updates from Alpaca. "
    ...
    async def cancel_all(self, timeout_seconds: float):
        ...
        for order in self._in_flight_orders.values():
            await order.get_exchange_order_id()
        tracked_orders: Dict[str, AlpacaInFlightOrder] = self._in_flight_orders.copy().items()
        ...
        try:
            ...
            for _, order in tracked_orders:
                api_params = {
                    "symbol": alpaca_utils.convert_to_exchange_trading_pair(order.trading_pair),
            ...
            self.logger().network(
                ...
                exc_info=True,
                app_warning_msg="Failed to cancel all orders on Alpaca. Check API key and network connection."
            )
        ...
        while True:
            try:
                ...
                self.logger().network(
                    ...
                    exc_info=True,
                    app_warning_msg="Could not fetch user events from Alpaca. Check API key and network connection."
                )
        ...
        for response in responses:
            for order in response["data"]["orders"]:
                ...
                ret_val.append(
                    OpenOrder(
                        client_order_id=tracked_order.client_order_id,
                        trading_pair=alpaca_utils.convert_from_exchange_trading_pair(order["symbol"]),
