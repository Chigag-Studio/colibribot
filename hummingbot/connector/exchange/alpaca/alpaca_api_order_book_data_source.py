...
class AlpacaAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def get_last_traded_prices(cls,
        ...
        :param domain: which Alpaca domain we are connecting to (the default value is 'com')
    ...
    async def get_all_mid_prices(domain="com") -> Dict[str, Decimal]:
        ...
        rest_assistant = await local_api_factory.get_rest_assistant()
        throttler = AlpacaAPIOrderBookDataSource._get_throttler_instance()

        url = alpaca_utils.public_rest_url(path_url=CONSTANTS.TICKER_PRICE_CHANGE_PATH_URL, domain=domain)
        ...
        for record in resp_json:
            try:
                pair = await AlpacaAPIOrderBookDataSource.trading_pair_associated_to_exchange_symbol(
    ...
    async def exchange_symbol_associated_to_pair(
        ...
        """
        symbol_map = await AlpacaAPIOrderBookDataSource.trading_pair_symbol_map(
            domain=domain,
    ...
    async def fetch_trading_pairs(domain="com") -> List[str]:
        ...
        """
        mapping = await AlpacaAPIOrderBookDataSource.trading_pair_symbol_map(domain=domain)
    ...
    async def get_new_order_book(self, trading_pair: str) -> OrderBook:
        ...
        snapshot_msg: OrderBookMessage = AlpacaOrderBook.snapshot_message_from_exchange(
            snapshot,
            snapshot_timestamp,
    ...
    async def listen_for_trades(self, ev_loop: asyncio.AbstractEventLoop, output: asyncio.Queue):
        ...
        while True:
            try:
                ...
                trade_msg: OrderBookMessage = AlpacaOrderBook.trade_message_from_exchange(
        ...
        while True:
            try:
                ...
                trading_pair = await AlpacaAPIOrderBookDataSource.trading_pair_associated_to_exchange_symbol(
                    ...
                    api_factory=self._api_factory,
                    throttler=self._throttler)
                order_book_message: OrderBookMessage = AlpacaOrderBook.diff_message_from_exchange(
    ...
    async def get_snapshot(
        ...
        url = alpaca_utils.public_rest_url(path_url=CONSTANTS.SNAPSHOT_PATH_URL, domain=self._domain)
    ...
    async def _init_trading_pair_symbols(
        ...
        rest_assistant = await local_api_factory.get_rest_assistant()
        local_throttler = throttler or cls._get_throttler_instance()
        url = alpaca_utils.public_rest_url(path_url=CONSTANTS.EXCHANGE_INFO_PATH_URL, domain=domain)
        request = RESTRequest(method=RESTMethod.GET, url=url)
        ...
        try:
            async with local_throttler.execute_task(limit_id=CONSTANTS.EXCHANGE_INFO_PATH_URL):
                ...
                if response.status == 200:
                    data = await response.json()
                    for symbol_data in filter(alpaca_utils.is_exchange_information_valid, data["symbols"]):
