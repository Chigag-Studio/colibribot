...
select_version () {
 echo
 echo
 echo "===============  UPDATE CBOT INSTANCE ==============="
 echo
 echo
 echo "ℹ️  Press [ENTER] for default values:"
 echo
 read -p "   Enter Cbot version to update [latest/development] (default = \"latest\") >>> " TAG
 if [ "$TAG" == "" ]
 then
   TAG="latest"
 fi
...
list_instances () {
 ...
 echo
 docker ps -a --filter ancestor=daldiv/cbot:$TAG
 echo
 ...
 echo
 echo "ℹ️  TIP: Connect to a running instance using \"./start.sh\" command and \"exit\" from inside Cbot."
...
execute_docker () {
 ...
 echo
 # 2) Delete old image
 docker image rm daldiv/cbot:$TAG
...
then
 ...
 declare -a INSTANCES
 INSTANCES=( $(docker ps -a --filter ancestor=daldiv/cbot:$TAG --format "{{.Names}}") )
 list_dir
 declare -a FOLDERS
 prompt_folder
 confirm_update
