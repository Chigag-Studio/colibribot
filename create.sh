...
read -p "   Enter Cbot version you want to use [latest/development] (default = \"latest\") >>> " TAG
if [ "$TAG" == "" ]
then
  TAG="latest"
fi
...
read -p "   Enter a name for your new Cbot instance (default = \"cbot-instance\") >>> " INSTANCE_NAME
if [ "$INSTANCE_NAME" == "" ]
then
  INSTANCE_NAME="cbot-instance"
  DEFAULT_FOLDER="cbot_files"
else
  DEFAULT_FOLDER="${INSTANCE_NAME}_files"
fi

# Ask the user for the folder location to save files
read -p "   Enter a folder name where your Cbot files will be saved (default = \"$DEFAULT_FOLDER\") >>> " FOLDER
if [ "$FOLDER" == "" ]
then
...
create_instance () {
 echo
 echo "Creating Cbot instance ... Admin password may be required to set the required permissions ..."
 echo
